...
const RezervareNouaATV4 = () => {
  ...
  useEffect(() => {
    ...
    if (diffHrs >= 24) {
      calculatedSumaRon = Math.floor(diffHrs / 24) *  600+ (diffHrs % 24 > 0 ? (diffHrs % 24) * 50 : 0);
    } else {
      calculatedSumaRon = diffHrs * 150;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("Tipul produsului este: " + newReservation.atv4Model)
    ...
    try {
        await axios.post('http://localhost:3001/rezervari/atv4', newReservation);
  ...
  return (
    ...
    <div className='form-group'>
          <label htmlFor='atv4Model'>Model Vehicul</label>
          <select
            name='atv4Model'
            id='atv4Model'
            value={product}
          ...
          >
            <option value='ATV2'>ATV CF Moto CForce 450L</option>
...
export default RezervareNouaATV4;